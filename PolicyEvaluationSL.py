...
class PolicyEvaluationSL():
    ...
    def __init__(self, periods, lifetime, holding_c, penalty_c, order_c, outdating_c, perish_prob, discount_factor, demand_params, demand_truncation, num_cores, FIFO_rate):
        ...
        if self.params[0] == 'Poisson':
            self.distr_cdf = {x: sp.poisson(self.params[1][0]).cdf(x) for x in range(self.max_d*self.m)}
        elif self.params[0] == 'NegBin':     
            # Generate new random negative binomial 
            self.distr_cdf = {x: sp.nbinom(self.params[1][0], self.params[1][1]).cdf(x) for x in range(self.max_d*self.m)}
    ...
    def terminal_state(self):
        ...
        """
        # Assign terminal cost (which would just be 1 as we satisfy all demand at the end as theres.... no demand.)
        for x in self.state_space:
            self.V.append((list(x), 1))
        return

    def calc_immediate_cost(self, x):
        return self.distr_cdf[np.sum(x)]
    ...
    def calculate_cost_to_go(self, x,q, V_t_plus_1):
        ...
        fut = self.calc_future_cost(x,q,V_t_plus_1)
        fut_cost = self.gamma*fut
    ...
    def EvalPolicy(self, policy):
        ...
        if(self.save_output):
            ext = self.params[0] + str(self.p) + str(self.theta)

            file = open('/beegfs/client/default/loweryb/sustainability/evaluation/SL/' + ext + '/testing_sl_rate_' + str(self.fifo_rate)  +'_with_policy_'+ str(self.input_policy_rate)  + '.pkl','wb')
